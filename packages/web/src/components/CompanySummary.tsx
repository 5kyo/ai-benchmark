  ...
  return (
    <div className="panel p-4">
      <div className="mb-3 flex items-baseline gap-2">
        ...
        <span className="text-xs" style={{ color: "var(--muted)" }}>근거 기반 해석</span>
      </div>
      ...
      <div className="space-y-5">
        ...
        <section>
          <h3 className="mb-2 flex items-center gap-2 text-sm font-semibold">
            <span style={{ color: "var(--score-high)" }}>▲</span> 강점
          </h3>
          {strengths.length === 0 ? (
            ...
            <ul className="space-y-2.5">
              {strengths.map((s) => (
                <li key={`${s.axis}-${s.metricKey}`}>
                  ...
                  {s.evidence && (
                    <p className="mt-0.5 text-xs leading-snug" style={{ color: "var(--muted)" }}>{s.evidence}</p>
                  )}
                </li>
              ))}
            </ul>
        ...
        <section>
          <h3 className="mb-2 flex items-center gap-2 text-sm font-semibold">
            <span style={{ color: "var(--score-low)" }}>▼</span> 개선 우선순위
            {weakIsFallback && (
          ...
          <ul className="space-y-3">
            {weaknesses.map((w) => (
              <li key={`${w.axis}-${w.metricKey}`}>
                ...
                {w.evidence && (
                  <p className="mt-0.5 text-xs leading-snug" style={{ color: "var(--muted)" }}>{w.evidence}</p>
                )}
                {w.suggestion && (
                  <p className="mt-0.5 text-xs leading-snug" style={{ color: "var(--signal)" }}>→ {w.suggestion}</p>
                )}
              </li>
            ))}
          </ul>
